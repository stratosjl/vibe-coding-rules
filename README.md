...
| **T4** mission-critical regulated | operator-set only (`tier: T4` in CLAUDE.md or `/vc-roe:tier T4`); auto-detection caps at T3 | **session-open cross-session audit** before any substantive work (scope, decisions and open issues across ALL sessions, enumerated from the commit history rather than the handover directory, denominator reported, gaps surfaced to the operator first); **eleven-element structural close**: (1) regression floor verified, **with its denominator stated: what the floor examined, not only that it ran and its exit code** + (2) change log entry + (3) runbook update + (4) compliance trace + (5) public-contract verification (lint clean) + (6) architecture decisions captured + (7) architecture-section row updated + (8) next-handover skeleton + (9) user-facing docs reviewed + (10) final smoke check + (11) change log written last; mid-flight pause sub-procedure (significant scope shift = stop + document + re-confirm with operator); multi-module review cadence for the project's own methodology document; pre-drafted handover skeletons for next N sessions if on a known phase trajectory; audit-trail target widens to 10-30 KB with mandatory one-paragraph TL;DR at top |
...
- **T4** adds the *eleven-element structural close* because regulator-compellable artefacts demand provability after the fact: every claim made at close needs evidence on disk, not just chat history. *Mid-flight pause* exists because at T4, an unannounced scope shift mid-session can leave a partial regulator-reportable state that's worse than no work at all. The *session-open cross-session audit* exists because every other completeness check in the stack compares one session's handover against the register and therefore assumes the register is complete; where a register file is missing, or a session ended without writing one, those checks pass over an empty set and report success.
...
T2+ tiers run a session-health heartbeat every ~30 min wall-clock. The hook layer keeps the clock:
...
Since v1.20.0 the tag is **informational**. When the cadence has elapsed the tag reads `DUE` (or `DUE-2X`), which is a clock state rather than a violation: the hooks no longer demand a heartbeat block and no longer read a missing sentinel as a failure. The reason is measured, not stylistic. Over a full 78-day hook log the sentinel was absent in 849 of 1,507 Stop checks (56.3%) and the tag read `OVERDUE` 45.7% of the time, so the demand was firing far more often than it was ever honoured, which is how a signal teaches its reader to ignore it.

To put the old enforcement back, set `VC_ROE_HEARTBEAT_ENFORCEMENT=1` in the environment (or flip `HEARTBEAT_ENFORCEMENT_DEFAULT` to `True` in `hooks/user-prompt-submit.py`, `hooks/post-tool-use.py`, `hooks/stop.py` and `hooks/kimi/stop.py`). The demand text and the Kimi-side turn-end block are kept intact behind that switch. The cadence itself is one constant, `CADENCE_SEC`, carried in the same four files.
